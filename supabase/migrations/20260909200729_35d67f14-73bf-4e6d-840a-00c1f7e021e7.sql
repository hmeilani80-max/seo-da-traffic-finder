...
-- 4. Additive workspace_id on existing project-scoped tables ----
...
-- 5. Additive Project lifecycle fields (existing status untouched)
...
-- 6. Deterministic default workspace + membership backfill ------
DO $$
DECLARE
  ws_id uuid;
BEGIN
  ...
  -- Only known existing app users that already own Wave 0 project data.
  ...
  -- Attach existing rows to the default workspace (user_id preserved).
...
-- 7. Additive workspace-membership RLS (owner policies preserved)
