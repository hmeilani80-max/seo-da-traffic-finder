...
function parseSseOrJson(text: string): JsonRpcEnvelope {
  ...
  if (!trimmed) {
    throw new Error("OpenSEO mengembalikan response kosong.");
  }
...
async function callOpenSeoTool(
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error("OpenSEO API key tidak valid atau tidak memiliki akses ke project.");
    }
...
function mapRankedKeyword(item: unknown): RankedKeyword | null {
  const keyword = readPath(item, "keyword_data", "keyword") ?? readPath(item, "keyword");
  ...
  const searchVolume = readPath(item, "keyword_data", "keyword_info", "search_volume");

  const cpc = readPath(item, "keyword_data", "keyword_info", "cpc");
  ...
  return {
    ...
    searchVolume: toNumber(searchVolume),
 ...
 * dari OpenSEO.
 */
async function getRankedKeywords(apiKey: string, projectId: string, target: string, limit: number) {
  ...
  return raw.map(mapRankedKeyword).filter((row): row is RankedKeyword => row != null);
}
...
async function getDomainOverview(
  ...
  const toolResult = await callOpenSeoTool(apiKey, "get_domain_overview", {
    projectId,
    domain,
    scope: "domain",
  });
  ...
  return {
    organicTraffic: toNumber(data["organicTraffic"]),

    organicKeywords: toNumber(data["organicKeywords"]),

    backlinks: toNumber(data["backlinks"]),

    referringDomains: toNumber(data["referringDomains"]),
  };
}
...
function sourceTokenWeights(rows: RankedKeyword[]) {
  ...
  for (const row of rows.slice(0, 40)) {
    const volumeWeight = Math.log10((row.searchVolume ?? 0) + 10);

    const trafficWeight = Math.log10((row.traffic ?? 0) + 10);

    const positionBoost = row.position != null && row.position <= 10 ? 1.15 : 1;

    const rowWeight = (volumeWeight * 0.55 + trafficWeight * 0.45) * positionBoost;

    const uniqueTokens = new Set(tokenize(row.keyword));
 ...
 */
function topicalRelevance(candidate: RankedKeyword, sourceWeights: Map<string, number>) {
  ...
  if (candidate.url) {
    try {
      pagePath = new URL(candidate.url).pathname;
    } catch {
      pagePath = candidate.url;
    }
  }

  const keywordTokens = tokenize(candidate.keyword);

  const pageTokens = tokenize(pagePath);

  const tokens = new Set([...keywordTokens, ...pageTokens]);
  ...
  for (const token of tokens) {
    matched += sourceWeights.get(token) ?? 0;
  }

  const denominator = Math.max(1, Math.min(tokens.size, 4));

  const normalized = matched / denominator;
...
function buildReason(input: {
  ...
  if (input.row.position != null) {
    ...
    parts.push("Keyword memiliki potensi sebagai target backlink.");
  }

  if (input.topical >= 0.7) {
    parts.push("Topiknya sangat relevan dengan domain sumber.");
  } else if (input.topical >= 0.45) {
    parts.push("Topiknya cukup relevan dengan domain sumber.");
  }

  if (input.pageCount === 0) {
    parts.push("Target page belum digunakan pada histori backlink.");
  } else if (input.diversity >= 0.55) {
    parts.push(`Target page baru digunakan ${input.pageCount}× di histori.`);
  }

  if (input.anchorCount === 0) {
    parts.push("Anchor belum pernah digunakan sebelumnya.");
  }
...
function scoreSuggestions(input: {
  ...
  for (const item of input.history) {
    ...
    if (target) {
      pageCounts.set(target, (pageCounts.get(target) ?? 0) + 1);
    }

    if (keyword) {
      anchorCounts.set(keyword, (anchorCounts.get(keyword) ?? 0) + 1);
    }
  }
  ...
  const scored = candidates
    ...
    .map((row) => {
      ...
      const normalizedPage = normalizeText(targetPage);

      const normalizedAnchor = normalizeText(row.keyword);

      const pageCount = pageCounts.get(normalizedPage) ?? 0;

      const anchorCount = anchorCounts.get(normalizedAnchor) ?? 0;
       ...
       */
      const trafficScore = clamp(Math.log10((row.traffic ?? 0) + 1) / Math.log10(maxTraffic + 1));
       ...
       * 15% Link Strength
       */
      let score = topical * 0.35 + opportunity * 0.3 + diversity * 0.2 + strength * 0.15;
       ...
       */
      try {
        const pathname = new URL(targetPage).pathname;
      ...
      return {
        ...
        searchVolume: row.searchVolume != null ? Math.round(row.searchVolume) : null,

        keywordDifficulty: row.keywordDifficulty != null ? Math.round(row.keywordDifficulty) : null,
 ...
 */
export const suggestBacklinkPlacements = createServerFn({
  method: "POST",
})
  .inputValidator(
    (data: { domain: string; sourceDr?: number | null; history?: BacklinkHistoryItem[] }) => {
      ...
      const history = Array.isArray(data.history)
        ? data.history.slice(0, 500).map((item) => ({
            keyword: typeof item?.keyword === "string" ? item.keyword.slice(0, 300) : null,

            targetPage:
              typeof item?.targetPage === "string" ? item.targetPage.slice(0, 2048) : null,
          }))
        : [];
  ...
  .handler(async ({ data }): Promise<BacklinkSuggestionResult> => {
    ...
    const projectId = process.env["OPENSEO_PROJECT_ID"]?.trim() || DEFAULT_PROJECT_ID;
    ...
    try {
       ...
       * jika call-nya gagal.
       */
      const [sourceRows, targetRows, targetOverview] = await Promise.all([
        getRankedKeywords(apiKey, projectId, data.domain, 40),

        getRankedKeywords(apiKey, projectId, targetDomain, 100),

        getDomainOverview(apiKey, projectId, targetDomain).catch((error) => {
          console.warn("[OpenSEO Domain Overview]", error);
      ...
      return {
        ...
        error: error instanceof Error ? error.message : "Gagal mengambil rekomendasi dari OpenSEO.",
      };
    }
  });
