...
import { ArrowUpDown, Download, Pencil, Sparkles, Trash2 } from "lucide-react";
...
export function DomainTable({
  ...
  const [suggestionRow, setSuggestionRow] = useState<DomainRow | null>(null);
  ...
  const totalCols = 7 + (detail ? 4 : 0) + (showSearchVolume ? 1 : 0);
  ...
  function ResearchStatus({ row }: { row: DomainRow }) {
    const status =
      row.research_status ?? (row.dr != null || row.traffic != null ? "selesai" : "belum_diriset");
    ...
    return (
      <Badge
        variant={
          status === "gagal" ? "destructive" : status === "selesai" ? "default" : "secondary"
        }
      >
        {labels[status]}
      </Badge>
  ...
  return (
    <div className="rounded-xl border bg-card shadow-[var(--shadow-card)]">
            ...
            <TableRow>
              ...
              {detail && (
                <>
                  ...
                  {showSearchVolume && <TableHead>Search Volume</TableHead>}
              ...
              <TableRow key={r.id}>
                ...
                {detail && (
                  <>
                    ...
                    {showSearchVolume && (
                      <TableCell>
                        {r.search_volume != null ? formatNumber(Number(r.search_volume)) : "-"}
                      </TableCell>
                    )}
          ...
          {detail && (
            <TableFooter>
              <TableRow className="font-semibold">
                <TableCell colSpan={totalCols - 3} className="text-right">
                  TOTAL HARGA
                </TableCell>
      ...
      {suggestionRow && table === "sudah_dibeli" && (
        <BacklinkSuggestionDialog
          row={suggestionRow}
          open={suggestionRow !== null}
          onOpenChange={(open) => {
            if (!open) {
