...
function OverviewMetric({ label, value }: { label: string; value: number | null | undefined }) {
  return (
...
export function BacklinkSuggestionPanel({
  ...
  domain: string;
  sourceDr: string;
  onUse: (suggestion: Pick<BacklinkSuggestion, "keyword" | "targetPage" | "searchVolume">) => void;
}) {
    ...
    mutationFn: async () => {
      ...
      const response = await runSuggestions({
        data: {
          ...
          sourceDr: parsedDr != null && Number.isFinite(parsedDr) ? parsedDr : null,
    ...
    <div className="rounded-xl border bg-muted/20 p-4 sm:col-span-2 lg:col-span-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            OpenSEO menganalisa keyword domain sumber, peluang SEO arsjadrasjid.com, histori
            backlink, dan kekuatan DR untuk memilih kombinasi keyword + target page.
          </p>
      ...
      {result && !result.error && result.targetOverview && (
        <div className="mt-4 rounded-lg border bg-card p-4">
          <div>
            <p className="text-sm font-semibold">OpenSEO Overview — arsjadrasjid.com</p>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-4">
            <OverviewMetric label="Organic Traffic" value={result.targetOverview.organicTraffic} />
            ...
            <OverviewMetric label="Backlinks" value={result.targetOverview.backlinks} />
        ...
        <div className="mt-4">
          <div className="mb-3">
            ...
            <p className="text-[11px] text-muted-foreground">
              Score dihitung dari 35% relevansi topik, 30% SEO opportunity, 20% backlink diversity,
              dan 15% link strength fit.
            </p>
          </div>
            ...
            {result.suggestions.map((item, index) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {index === 1 && <Badge variant="secondary">Alternative #2</Badge>}

                  {index === 2 && <Badge variant="secondary">Alternative #3</Badge>}

                  <Badge variant="outline">Score {item.score}/100</Badge>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <p className="mt-1 font-semibold leading-snug">{item.keyword}</p>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                  <div className="rounded-md bg-muted/40 p-2">
                    <p className="text-[10px] text-muted-foreground">Google Position</p>

                    <p className="mt-1 font-semibold">{formatNumber(item.rank)}</p>
                  </div>

                  <div className="rounded-md bg-muted/40 p-2">
                    <p className="text-[10px] text-muted-foreground">Search Volume</p>

                    <p className="mt-1 font-semibold">{formatNumber(item.searchVolume)}</p>
                  </div>

                  <div className="rounded-md bg-muted/40 p-2">
                    <p className="text-[10px] text-muted-foreground">Keyword Difficulty</p>

                    <p className="mt-1 font-semibold">{formatNumber(item.keywordDifficulty)}</p>
                  </div>

                  <div className="rounded-md bg-muted/40 p-2">
                    <p className="text-[10px] text-muted-foreground">Estimated Traffic</p>

                    <p className="mt-1 font-semibold">{formatNumber(item.trafficEstimate)}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  ...
                  <ScoreChip label="SEO Opportunity" value={item.seoOpportunity} />

                  <ScoreChip label="Diversity" value={item.backlinkDiversity} />

                  <ScoreChip label="Link Strength" value={item.linkStrengthFit} />
                </div>
      ...
      {result && !result.error && (
        <div className="mt-4 border-t pt-3">
          <p className="text-[11px] text-muted-foreground">
            Evidence: <strong className="text-foreground">{result.sourceKeywordsFound}</strong>{" "}
            ranked keyword domain sumber ·{" "}
            <strong className="text-foreground">{result.targetKeywordsFound}</strong> ranked keyword
            arsjadrasjid.com dianalisa. Hasil tetap dapat diedit manual sebelum disimpan.
          </p>
        </div>
      )}
    </div>
